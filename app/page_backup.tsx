        ...
        <div style={{ flex: 1 }}>
          <div style={{
            ...
            {message.content.split('\n').map((line, i) => (
              <span key={i}>
                {line.startsWith('## ') ? (
                  <strong style={{color:'#a78bfa',fontSize:'14px',display:'block',marginTop:'12px',marginBottom:'4px'}}>
                    {line.replace('## ','')}
                  </strong>
                ) : line.startsWith('- ') ? (
                  <span style={{display:'block',paddingLeft:'12px',marginBottom:'2px'}}>• {line.replace('- ','')}</span>
                ) : line.startsWith('**') ? (
                  <span style={{display:'block'}} dangerouslySetInnerHTML={{__html:line.replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>')}}/>
                ) : (
